...
const PinModal = ({ onChange, visible, transparent }: pinFirst) => {

  const router = useRouter();

  const [numCheck, setNumCheck] = useState(0); // 1차 입력된 비밀번호 개수
  const [pinCheck, setPinCheck] = useState<string>('') // 1차 6자리 비밀번호
  ...
  const touchCircle = circleNumber.map((number) => 
    ...
    return (
      ...
      <View className='flex-1 justify-center items-center' style={{ backgroundColor: 'rgba(0, 0, 0, 0.7)' }}>
        ...
        <Text className='text-2xl font-bold'>비밀번호 설정</Text>
